...
主 CIR 播放器的 RT 叠加默认开启：统计 median 使用青色，P5/P95 显示为边界，RT PDP 使用红色。播放器必须展示真实 frameId、UTC、RX 坐标、运动状态、帧间位移、仰角、斜距和三项拟合指标。环境、TEC 或已启用校准中的散射功率偏移变化后，当前 RT 与拟合结果必须立即失效并自动重算；重算期间统计 PDP 基础层仍须可见，RT 开关和三项拟合指标须暂时禁用，禁止把旧 RT 结果叠加到新参数上。
