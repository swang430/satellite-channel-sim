...
| 指标 | 说明 | 标准 | 单位 |
|------|------|------|------|
| Free Space Loss | 自由空间路径损耗 | Friis 方程 | dB |
| Rain Attenuation | 雨衰 | ITU-R P.838-3（19点对数线性插值） | dB |
| Gas Attenuation | 氧气 + 水蒸气吸收 | ITU-R P.676-12 Annex 2 | dB |
| Cloud Attenuation | 云/雾液态水衰减 | ITU-R P.840-8（双Debye介电模型） | dB |
| C/N | 载噪比 | — | dB |
| XPD | 交叉极化鉴别度 | — | dB |
| Faraday Rotation | 电离层法拉第旋转角 | ITU-R P.531 | deg |
| Group Delay | 电离层群延迟 | TEC / f² | ns |

### 关键参数

| 参数 | 说明 | 典型范围 | 默认值 |
|------|------|---------|--------|
| Freq (GHz) | 工作频率 | 0.3 ~ 100 | 30 |
...
系统使用三套 ITU-R 标准来计算大气损耗：

**雨衰（ITU-R P.838-3）**

采用 19 个频率点（1–100 GHz）的完整查找表，在对数频率域上做线性插值求系数 k 和 α，精度远高于简单查表取最近邻。路径模型采用有效路径长度折减系数（雨高 3 km + 距离折减因子）。

**气体衰减（ITU-R P.676-12 Annex 2）**

分别计算氧气特定衰减 γ_o（含 φ 系数修正）和水蒸气特定衰减 γ_w（含 22.235 GHz 共振线），按有效路径高度积分（h_O = 6 km，h_W = 2.1 km）后用余弦斜距折算。标准大气（T=15°C，p=1013.25 hPa，ρ=7.5 g/m³）下典型值：
...
使用双 Debye 介电模型计算液态水特定衰减系数 K_l（温度相关，默认云温 0°C），柱状液水含量默认 L = 0.5 kg/m²（中纬度典型值）。公式：A_c = K_l × L / sin(θ)。
...
| Chart.js / react-chartjs-2 | 4.5 / 5.3 | MIT |
| satellite.js | 6.0 | MIT |
| JSZip | 3.10 | MIT / GPL-3.0 双协议（选用 MIT） |
| mathjs | 15.1 | Apache-2.0 |
| mat-for-js | 1.1 | GPL-3.0-or-later |
| Vite | 7.3 | MIT（仅构建） |

**本项目整体采用 GPL-3.0-or-later 协议发布**（因运行时依赖 mat-for-js 的 GPL-3.0 协议要求）。详见项目根目录 `LICENSE` 文件。
